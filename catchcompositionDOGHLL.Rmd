...
samps |>
  group_by(survey_abbrev) |> 
  mutate(group = ifelse(sex ==2 & length < 77, "immature", 
         ifelse(sex == 1 & length < 65, "immature", "notimmature"))) |> 
  filter(group %in% c("immature")) |> 
  group_by(year, group, survey_abbrev) |> 
  mutate(tally = n()) |> 
  ggplot() +  
  geom_line(aes(year, tally, group = survey_abbrev, colour = survey_abbrev)) 
...
samps |> 
  group_by(sex, survey_abbrev) |> 
  summarise(minlength = min(length), maxlength = max(length))

x <- samps |> 
  group_by(sex, survey_abbrev, year) |> 
  filter(length < 50) |> 
  summarize(sum = n()) 

samps2 <- samps |> 
  mutate(lengthgroup = ifelse(sex == 2 & length >95, "maturef", 
         ifelse(sex == 2 &  length < 77, "immature",  
         ifelse(sex == 2 &  length >= 77 & length <= 95, "maturingf", 
         ifelse(sex == 1 &  length > 77, "maturem", 
         ifelse(sex == 1 &  length < 65, "immature",  "maturingm"))))))

samps2 |> 
  group_by(sex, survey_abbrev, year, lengthgroup) |> 
  filter(year > 2000) |> 
  summarize(sum = n()) |> 
  ggplot() +  
  geom_point(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) + 
  geom_line(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) + facet_wrap(~lengthgroup + survey_abbrev, scales = "free")

samps2 |> 
  filter(lengthgroup == "immature") |> 
  group_by(survey_abbrev, year) |> 
  filter(year > 2000) |> 
  summarize(sum = n()) |> 
  ggplot() +  
  geom_point(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) + 
  geom_line(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) 
  + facet_wrap(~lengthgroup + survey_abbrev, scales = "free")
...
samps2 |> 
  ...
  ggplot() +  
  #geom_point(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) + 
  geom_line(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) + facet_wrap(~lengthgroup , scales = "free")
...
samps |> 
  ...
  ggplot() +  
  geom_point(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) + 
  geom_line(aes(year, sum, group = survey_abbrev, colour = survey_abbrev)) + facet_wrap(~survey_abbrev, scales = "free")
...
geom_jitter(
  ...
  geom_hline(yintercept = 77, col = "red")+ 
