...
- [1]	Do extra depths need to be added to the HBLL survey to capture an sex/length composition to create an index? Evaluate data as surveys are completed to understand the catch size and sex distribution. Seems that dogfish are shallower in summer and it’s possible that we are capturing the same size and sex distribution at shallower depths. However, we only have data from three sites and the results are variable. Dogfish move deeper in the later months. Also potential for differences in the catchability in individuals between seasons?
...
Details of the comparative surveys. 2019 HBLL with DOG circle hooks run at x stations. 2022 comparative work included HBLL and DOG circle hooks, same line, run at x sites. 2004 work was completed during the DOG survey and included fishing sits twice, once with J-hooks and once with circle hooks (ref:). 
...
sets <- readRDS("C:/Dogfish surveys 2022/output/dogfish_sets.rds")

```


To access the data, I used a sql command and include those surveys completed with circle hooks `unique(sets$survey_series_desc)` in years `unique(sets$years)`. A calibration survey between the J-hook and circle hooks was completed in 2004. Calibrating those catches to extend the timeseries will be completed shortly. 
```{r sqlfromsean}
...
saveRDS(d, "C:/Dogfish surveys 2022/output/dogfishsql_sets.rds")
```
...
saveRDS(dog, "C:/Dogfish surveys 2022/output/dogfishsql_sets.rds")
...
```{r}
d <- readRDS("C:/Dogfish surveys 2022/output/dogfishsql_sets.rds")

unique(d$survey_series_id)
sort(unique(d$year))

test <- filter(d, survey_series_id == 76 )# no 1986, 1989,2004,  2022
unique(test$survey_series_desc)
unique(sort(test$year))
...
Note: hooks fished is variable across surveys. 
...
saveRDS(dog2, "C:/Dogfish surveys 2022/output/dogfishdata.rds")

```

Names are fixed. 
```{r checknames}
dog <- readRDS("C:/Dogfish surveys 2022/output/dogfishdata.rds")
...
Look at length distributions of the HBLL and DOG dogfish surveys. Used `get_survey_samples` to pull length information. Surveys included: `samps |> distinct(year, survey_series_desc) |> print (n= 30)`. 
```{r loadsamps, eval = FALSE}
...
dogsamps <- filter(allsamps, survey_series_desc %in% c("Hard Bottom Longline Inside North ",   
                                                      "Strait of Georgia Dogfish Longline", 
                                                "Hard Bottom Longline Inside South " ))

saveRDS(dogsamps, "C:/Dogfish surveys 2022/output/dogfish_samps.rds")
```


```{r loadsamps}
samps <- readRDS("C:/Dogfish surveys 2022/output/dogfish_samps.rds")
...
# samps |> 
#   distinct(year, survey_series_desc) |> 
#   print (n= 30)

```

Density plots of lengths distributions for males and females. 
```{r densityplots}

ggplot() + 
  geom_density(data = samps, aes(length, group = as.factor(survey_series_desc), 
...
ggplot() +
  ...
  #            colour = as.factor(survey_series_desc), alpha = 0.15)) + 
   geom_jitter(data = samps, aes(survey_series_desc, length, group = as.factor(survey_series_desc), 
                                 fill = as.factor(survey_series_desc)), 
               alpha = 0.15, size = 0.5, colour = "grey10") +
  geom_violin(data = samps, aes(survey_series_desc, length, group = as.factor(survey_series_desc), 
                                 fill = as.factor(survey_series_desc)), 
...
ggplot() +
  #geom_jitter(data = samps, aes(year, length, 
  #            colour = as.factor(survey_series_desc), alpha = 0.15)) + 
  geom_boxplot(data = filter(samps, sex %in% c(1,2)), aes(as.factor(year), length, 
                                 fill = as.factor(survey_series_desc)), 
               alpha = 0.35, size = 1, colour = "black") + 
...
Create an index using the HBLL and DOG surveys. 
```{r}
#1. check sets data
dog <- readRDS("C:/Dogfish surveys 2022/output/dogfishdata.rds")
dog$logbot_depth <- log(dog$depth_m)
ggplot(dog, aes(longitude, latitude, colour = location)) + geom_point() + facet_wrap(~year)
dog <- add_utm_columns(dog,
  ll_names = c("longitude", "latitude"), units = "km",
  utm_crs = 32609
)
mean_logbot_depth <- mean(dog$logbot_depth)
...
grid <- add_utm_columns(grid2, units = "km", utm_crs = 32609) |>
  # rename("UTM.lon" = "X", "UTM.lat" = "Y") |>
  # mutate(UTM.lon = round(UTM.lon, 2),
  mutate(
    X = round(X, 2),
    area_swept = 1,
    months_from_solstice = 0
  )
...
grid$area_km <- as.numeric(grid$area_km)

sort(unique(grid$year))
sort(unique(grid$year_scaled))
sort(unique(grid$area_km))
sort(unique(grid$logbot_depth))
sort(unique(grid$logbot_depth))
