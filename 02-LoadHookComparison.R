...
# 2019 HBLL strait of georgia dogfish survey ----------------------------------------
# Calibration survey  - 2004
# Hook code - is one of them in there, hook code designates if it was circle or j hook,
# newers years are 1 and the others are 3, and one year that has both (that is the experimental year).
...
dogcomp_info2004 %>%
  group_by(fe_fishing_ground_comment) %>%
  tally()
...
test <- dog_longlinespecs %>%
  filter(!is.na(fe_parent_event_id)) %>%
...
x <- dog_longlinespecs2 %>% dplyr::inner_join(dogcomp_info2004, by = c("fe_parent_event_id" = "fishing_event_id"))
length(unique(x$fishing_event_id))
length(unique(x$fe_parent_event_id))
anti <- dog_longlinespecs2 %>% dplyr::anti_join(dogcomp_info2004, by = c("fe_parent_event_id" = "fishing_event_id"))
anti <- dogcomp_info2004 %>% dplyr::anti_join(dog_longlinespecs2, by = c("fishing_event_id" = "fe_parent_event_id"))
...
ggplot(hbll_dogfish_catch2, aes(depth_m, dogfish_count / lglsp_hook_count, group = as.factor(GearType), colour = as.factor(GearType))) +
  geom_point(size = 2) +
  geom_line(size = 2) +
...
ggplot(hblldog, aes(longitude, latitude)) +
  geom_point() +
  geom_label(aes(label = fishing_event_id))
...
ggplot(augdog, aes(latitude, longitude, colour = as.factor(site))) +
  geom_point()
ggplot(octdog2, aes(latitude, longitude, colour = as.factor(site))) +
  geom_point()
...
ggplot(dog5, aes(depth_m, dogfish_count / lglsp_hook_count, group = as.factor(survey_gear), colour = as.factor(survey_gear))) +
  geom_point(size = 2) +
  facet_wrap(~sitename) +
  geom_line(size = 2) +
  scale_colour_manual(values = c("#6F99ADFF", "#7876B1FF", "#E18727FF"))
...
m_dog <- glmer(
  dogfish_count ~ poly(depth_m, 2) * as.factor(survey_desc2) + offset(log(lglsp_hook_count)) + (1 + survey_desc2 | site),
  data = dog3,
...
nd <- expand.grid(
  depth_m = seq(min(dog3$depth_m), max(dog3$depth_m), length.out = 30),
...
ggplot(test, aes(grouping_desc, Total_Length, group = ACTIVITY_DESC, colour = ACTIVITY_DESC)) +
  geom_jitter(size = 2) +
  facet_wrap(~ SPECIMEN_SEX_CODE + site)

ggplot(test, aes(grouping_desc, Total_Length, group = ACTIVITY_DESC, colour = ACTIVITY_DESC)) +
  geom_jitter(aes(group = ACTIVITY_DESC), size = 2) +
  facet_wrap(~ SPECIMEN_SEX_CODE + site)
...
ggplot(test, aes(grouping_desc, Total_Length)) +
  ...
  geom_boxplot(aes(fill = ACTIVITY_DESC)) +
  facet_wrap(~ SPECIMEN_SEX_CODE + site) +
...
test <- dsurvey_bio2 %>%
  group_by(ACTIVITY_DESC, SPECIMEN_SEX_CODE) %>%
  summarise(numsex = n(), meanlength = mean(Total_Length), medianlength = median(Total_Length), maxlength = max(Total_Length), minlength = min(Total_Length))
